/** @type {import('tailwindcss').Config} */
export default {
    content: [
        "./index.html",
        "./src/**/*.{ts,html,css}",
        "../ora-components/src/**/*.{ts,html,css}",
    ],
    darkMode: ['selector', '[data-theme="dark"]'],
    theme: {
        extend: {
            colors: {
                primary: 'var(--md-sys-color-primary)',
                'on-primary': 'var(--md-sys-color-on-primary)',
                'primary-container': 'var(--md-sys-color-primary-container)',
                'on-primary-container': 'var(--md-sys-color-on-primary-container)',
                secondary: 'var(--md-sys-color-secondary)',
                'on-secondary': 'var(--md-sys-color-on-secondary)',
                'secondary-container': 'var(--md-sys-color-secondary-container)',
                'on-secondary-container': 'var(--md-sys-color-on-secondary-container)',
                tertiary: 'var(--md-sys-color-tertiary)',
                'on-tertiary': 'var(--md-sys-color-on-tertiary)',
                'tertiary-container': 'var(--md-sys-color-tertiary-container)',
                'on-tertiary-container': 'var(--md-sys-color-on-tertiary-container)',
                error: 'var(--md-sys-color-error)',
                'on-error': 'var(--md-sys-color-on-error)',
                background: 'var(--md-sys-color-background)',
                'on-background': 'var(--md-sys-color-on-background)',
                surface: 'var(--md-sys-color-surface)',
                'on-surface': 'var(--md-sys-color-on-surface)',
                'surface-variant': 'var(--md-sys-color-surface-variant)',
                'on-surface-variant': 'var(--md-sys-color-on-surface-variant)',
                outline: 'var(--md-sys-color-outline)',
                'surface-container-low': 'var(--md-sys-color-surface-container-low)',
                'kpi-purple': 'var(--kpi-accent)',
                'kpi-accent': 'var(--kpi-accent)',
                'kpi-muted': 'var(--kpi-muted)',
                'kpi-rose': 'var(--kpi-rose)',
                'kpi-sky': 'var(--kpi-sky)',
                'kpi-green': 'var(--kpi-green)',
                'kpi-amber': 'var(--kpi-amber)',
                'kpi-pink': 'var(--kpi-pink)',
                'kpi-red': 'var(--kpi-red)',
                'kpi-violet': 'var(--kpi-violet)',
                'dashboard-accent': 'var(--dashboard-accent)',
                'dashboard-accent-soft': 'var(--dashboard-accent-soft)',
                'dashboard-border-soft': 'var(--dashboard-border-soft)',
                'trend-positive': '#16a34a',
                'trend-negative': '#dc2626',
                'trend-positive-bg': 'rgba(22,163,74,0.08)',
                'trend-negative-bg': 'rgba(220,38,38,0.08)',
            },
            fontSize: {
                'display-large': ['57px', { lineHeight: '64px', fontWeight: '600' }],
                'headline-large': ['32px', { lineHeight: '40px', fontWeight: '600' }],
                'headline-medium': ['28px', { lineHeight: '36px', fontWeight: '600' }],
                'headline-small': ['24px', { lineHeight: '32px', fontWeight: '600' }],
                'title-large': ['20px', { lineHeight: '28px', fontWeight: '600' }],
                'title-medium': ['16px', { lineHeight: '24px', fontWeight: '600' }],
                'title-small': ['14px', { lineHeight: '20px', fontWeight: '600' }],
                'body-large': ['16px', { lineHeight: '24px', fontWeight: '400' }],
                'body-medium': ['14px', { lineHeight: '20px', fontWeight: '400' }],
                'body-small': ['12px', { lineHeight: '16px', fontWeight: '400' }],
                'label-large': ['14px', { lineHeight: '20px', fontWeight: '600' }],
                'label-medium': ['12px', { lineHeight: '16px', fontWeight: '600' }],
                'label-small': ['11px', { lineHeight: '16px', fontWeight: '600' }],
            },
            borderRadius: {
                small: '4px',
                medium: '6px',
                large: '12px',
                'extra-large': '24px',
            },
            spacing: {
                'px-4': '4px',
                'px-8': '8px',
                'px-12': '12px',
                'px-16': '16px',
                'px-24': '24px',
                'px-32': '32px',
                'px-40': '40px',
                'px-48': '48px',
                'px-64': '64px',
                'px-96': '96px',
            },
            boxShadow: {
                'level-0': 'none',
                'level-1': '0px 1px 3px 1px rgba(0, 0, 0, 0.15), 0px 1px 2px 0px rgba(0, 0, 0, 0.30)',
                'level-2': '0px 2px 6px 2px rgba(0, 0, 0, 0.15), 0px 1px 2px 0px rgba(0, 0, 0, 0.30)',
                'level-3': '0px 4px 8px 3px rgba(0, 0, 0, 0.15), 0px 1px 3px 0px rgba(0, 0, 0, 0.30)',
                'level-4': '0px 6px 10px 4px rgba(0, 0, 0, 0.15), 0px 2px 3px 0px rgba(0, 0, 0, 0.30)',
                'level-5': '0px 8px 12px 6px rgba(0, 0, 0, 0.15), 0px 4px 4px 0px rgba(0, 0, 0, 0.30)',
            },
        },
    },
    plugins: [],
    safelist: [
        'text-kpi-purple',
        'text-kpi-accent',
        'text-kpi-muted',
        'text-kpi-rose',
        'text-kpi-sky',
        'text-kpi-green',
        'text-kpi-amber',
        'text-kpi-pink',
        'text-kpi-red',
        'text-kpi-violet',
        'text-dashboard-accent',
        'bg-dashboard-accent',
        'bg-dashboard-accent-soft',
        'border-dashboard-border-soft',
        'text-trend-positive',
        'text-trend-negative',
        'bg-trend-positive-bg',
        'bg-trend-negative-bg',
    ],
}
