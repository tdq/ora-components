 ...
 */
export const MountedLate = () => {
    const container = document.createElement('div');
    container.className = 'flex flex-col gap-4 p-8';

    const status = document.createElement('div');
    status.className = 'text-body-medium text-on-surface-variant py-16 text-center';
    status.textContent = 'Chart will mount in 1.5s (building + appending is deferred)...';
    ...
    setTimeout(() => {
        ...
        const builder = new ChartBuilder<DataItem>()
            .withData(of(data))
            .withCategoryField('month')
            .withTitle(of('Mounted 1.5s after story load'))
            .withHeight(400);
