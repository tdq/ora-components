...
import { getDaysInMonth, getFirstDayOfMonth, isSameDay, isValidDate } from '../date-picker/date-utils';
import { DayOfWeek } from '../date-picker/types';
...
export function renderCalendar(options: CalendarOptions): HTMLElement {
    ...
    // Initial sync
    subs.push(options.selectedDate$.pipe(take(1)).subscribe(syncView));

    // Sync when calendar expands
    if (options.isExpanded$) {
        subs.push(options.isExpanded$.subscribe(expanded => {
            if (expanded) {
                subs.push(options.selectedDate$.pipe(take(1)).subscribe(syncView));
            }
        }));
    ...
    subs.push(combineLatest([viewDate$, options.selectedDate$, focusedDate$, minDate$, maxDate$]).subscribe(([viewDate, selectedDate, focusedDate, minDate, maxDate]) => {
