...
export function createNumberFieldSupportText(id: string): HTMLElement {
    ...
    supportText.id = id;
    supportText.className = 'md-label-small transition-all duration-200 text-error px-px-16 mt-px-4 hidden';
...
export function createNumberFieldErrorIcon(errorText: string): HTMLElement {
    ...
    button.type = 'button';
    button.className = 'flex items-center justify-center w-px-24 h-px-24 rounded-full hover:bg-black/5 dark:hover:bg-white/5 transition-colors focus:outline-none ml-px-8';
    button.setAttribute('aria-label', `Error: ${errorText}`);
    button.innerHTML = `
        <svg viewBox="0 0 24 24" class="w-px-20 h-px-20 fill-current text-error">
    ...
    popover.className = 'error-popover bg-error text-on-error md-label-small px-px-12 py-px-8 rounded-small shadow-lg max-w-xs transition-opacity duration-200';
