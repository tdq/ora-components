...
The landing page fully integrates with the `@tdq/ora-components` Theme Management system:
...
The landing page uses **`RouterBuilder`** from `@tdq/ora-components` for client-side routing, defined in `src/routes.ts`:
...
## Integration with `@tdq/ora-components`
The landing page consumes `@tdq/ora-components` directly as a workspace dependency. It showcases the library's:
...
1. **Turbo dependency chain:** `turbo run build --filter=landing-page...` ensures `@tdq/ora-components` is built first (its `^build` dependency), then the landing page.
